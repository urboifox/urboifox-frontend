...
import { ArrowRight, Code, GoToLink } from "../../assets/icons";

export default function ProjectPage() {
  const allProjects = useSelector((state) => state.websiteData.data.work);
  const { id } = useParams();
  const project = allProjects?.find((e) => e.id === Number(id));
  const nextProject =
  ...
  return (
    <div className="projectPage flex flex-col items-center cont mx-auto pt-28 md:pt-32 relative">
      ...
      <div className="link max-w-lg mb-10 group md:mb-20 px-4 md:px-10">
        <p className="text-[var(--sec-color)] md:group-hover:text-[var(--main-color)] font-light text-sm md:text-lg tracking-wide leading-relaxed">
          {project?.description}
        </p>
      </div>
